    ...
    if (match) {
      return (
        <a
          ...
          className="'font-semibold text-afa-deep hover:underline'"
        >
          {part}
        </a>
      );
...
export default function ServicesGrid({ locale, phases, ctaLabel }: Props) {
  return (
    <section className="'w-full bg-afa-light py-20 lg:py-24'" aria-label="Service phases">
        ...
        <div className="space-y-16">
          {phases.map((phase) => (
            <article
              key={phase.id}
              id={phase.anchor}
              className="'scroll-mt-24 rounded-lg border border-afa-border bg-white p-8 md:p-10'"
            >
              ...
              {phase.decisionInputs && phase.decisionInputs.length > 0 && (
                <div className="'mt-6 rounded-lg border border-afa-border bg-afa-light p-5'">
              ...
              {phase.riskNotes && phase.riskNotes.length > 0 && (
                <div className="'mt-6 rounded-lg border border-afa-warning/20 bg-afa-warning/3 p-5'">
                  ...
                  <ul className="space-y-1.5">
                    {phase.riskNotes.map((note, idx) => (
                      <li key={idx} className="'flex items-start gap-2.5 text-sm'">
                        <span className="'shrink-0 mt-1 w-1.5 h-1.5 rounded-full bg-afa-warning'" />
                        <span>{note}</span>
                      </li>
                    ))}
                  </ul>
